 ...
 *
 * This is the official `sveltia/sveltia-cms-auth` worker
 * (https://github.com/sveltia/sveltia-cms-auth), vendored verbatim so the CMS
 * auth handshake matches Sveltia/Decap exactly. It brokers GitHub (and GitLab)
 * OAuth for the Sveltia admin at waai.me/admin, implementing the popup
 * `postMessage` contract (`authorizing:<provider>` → `authorization:<provider>:<state>:<json>`).
 *
 * Routes: GET /auth (start) · GET /callback (exchange + hand token to opener)
 * Required secrets: GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET
 * Optional: ALLOWED_DOMAINS (comma-separated, supports `*.example.com` wildcards),
 *           GITHUB_HOSTNAME (GitHub Enterprise Server only; defaults to github.com)
 */
