...
- **Hero phone = self-hosted promo video** — the desktop phone mockup's screen is a muted looping MP4 (the waai Short "waai Changes Everything"), NOT the old static chat mockup. Assets in `public/videos/`: `hero-waai-short.mp4` (720×1280 H.264 + AAC 96k audio, CRF 27, +faststart, ~3.0 MB / 24.6 s) + `hero-waai-short.jpg` poster (hand-picked sharp WhatsApp-chat-UI frame at 14 s — early frames are motion-blurred café scenes). **Source of truth: Peter's original file** (YouTube blocks all server-IP downloads — yt-dlp/embeds/cookies/Piped/Invidious all bot-walled; the original arrived via Virtualmin upload to `/home/waai/` and was re-encoded with ffmpeg). The `<video>` in `Hero.astro` has **no `autoplay` attribute** — an `is:inline` gate script plays it only when: viewport ≥1024px (mockup is `hidden lg:flex`; mobile fetches nothing beyond moov metadata), `prefers-reduced-motion` unset, and IntersectionObserver says on screen (same run-gate idea as GalaxyCanvas). `object-cover` crops 9:16 into the 264×556 screen. A **sound-toggle button** (`data-hero-sound`, bottom-right over the video, `home.hero.video.soundOn/soundOff` i18n keys in `en.ts` only — icon-only so EN fallback is fine) unmutes on click — the click is the user gesture autoplay policies need; click again re-mutes. The two floating stat badges sit `-left-24`/`-right-24` (96px off-frame; safe at 1024px — the 64px grid gap absorbs the overhang). Dead `home.hero.chat.*` keys were removed from all 10 locale dictionaries — the video is language-neutral.
