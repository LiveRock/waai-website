...
- **Messages/month `*` footnote** (pricing page only): the messages/month quota is prefixed with `*` on the plan cards (`msgLine`) and the comparison row label, explained by a footnote block after the comparison table — `*Within Customer Service Window (CSW)` + `*Within Free Entry Point (FEP)` as teal links to Meta's docs (`…/cloud-api/guides/send-messages#customer-service-windows`, `…/pricing#free-entry-point-windows`), then a bold "WhatsApp Marketing Messages" heading + charges disclaimer (`pricing.footnote.*` keys, all 10 locales). The `*` is added at the **PricingPage call sites, NOT in the `pricing.nMessages` dictionary value** — SignupForm has a copy-pasted `msgLine` on the same key and /signup intentionally stays unmarked
...
- **Build-time data sources** — both `/pricing` and `/signup` fetch inline in frontmatter (try `http://127.0.0.1:8000` then `https://waaichat.hsi.asia`; hardcoded fallback if unreachable):
  - `GET /api/public/plans` — plan prices + every quota (`max_qa_pairs`, `max_flows`, `max_messages_monthly`, `max_respondents`, `max_web_pages`, `max_drive_sends_monthly`, `max_qa_translations_per_lang_monthly`, `max_flow_translations_per_lang_monthly`, `max_bookings_monthly`, `max_booking_branches`, `max_booking_services`, `max_catalog_products`, `max_contacts`, `max_broadcasts_monthly`, `trial_days`). The comparison table surfaces most of these as quota rows; NULL renders as "Unlimited". (`max_broadcasts_monthly` row removed 2026-09-11 — broadcast sends are WhatsApp marketing messages, covered by the footnote disclaimer; the `pricing.row.broadcastsMonth` key was deleted from all 10 dictionaries.)
...
- **Hero phone = self-hosted promo video** — the phone mockup's screen is a muted looping MP4 (the waai Short "waai Changes Everything"), NOT the old static chat mockup. Assets in `public/videos/`: `hero-waai-short.mp4` (720×1280 H.264 + AAC 96k audio, CRF 27, +faststart, ~3.0 MB / 24.6 s) + `hero-waai-short.jpg` poster (hand-picked sharp WhatsApp-chat-UI frame at 14 s — early frames are motion-blurred café scenes). **Source of truth: Peter's original file** (YouTube blocks all server-IP downloads — yt-dlp/embeds/cookies/Piped/Invidious all bot-walled; the original arrived via Virtualmin upload to `/home/waai/` and was re-encoded with ffmpeg). The `<video>` in `Hero.astro` has **no `autoplay` attribute** — an `is:inline` gate script drives it: **desktop** (≥1024px) autoplays muted while on screen (IntersectionObserver) and motion is allowed (`prefers-reduced-motion` → static poster); **mobile** shows the poster + a `data-hero-play` overlay (`lg:hidden`, green play circle) — tap starts playback **with sound** (the tap is the user gesture autoplay policies require) and opts the video into pause-offscreen/resume-onscreen (`userStarted` flag). `object-cover` crops 9:16 into the 264×556 screen. A **sound-toggle button** (`data-hero-sound`, bottom-right over the video; `home.hero.video.*` i18n keys in `en.ts` only — icon-only so EN fallback is fine) toggles audio on desktop. The mockup column is visible on **all viewports** (stacks below the hero text on mobile); floating stat badges are `-left-6`/`-right-6` below `lg` (96px off-frame only on desktop). Dead `home.hero.chat.*` keys were removed from all 10 locale dictionaries — the video is language-neutral.
- **Hero secondary CTA = Book a Demo** — points at Peter's cal.com link (`https://cal.com/peter-rock-steady-crew/30min`), new tab via `CTAButton`'s `external` prop (adds `target="_blank" rel="noopener noreferrer"`). Label uses the `cta.bookDemo` key (translated in all 10 locales; was previously an unused dictionary entry). "Talk to Sales" / the contact form remain reachable via header nav.
- **Meta Tech Provider badge** — "Official Meta Tech Provider" (Meta's verbatim partner-badge wording — Peter confirmed matching the badge exactly) renders as a monochrome inline-SVG Meta loop + label in **three placements**: homepage hero pill (beside the "Your WhatsApp AI Agent" badge; wraps to a centered second row on mobile), the Footer brand column under the tagline (every `PageLayout` page — site-wide except trial LPs/404), and the `/trial` slim brand bar (`ml-auto`, all 100 LPs). One shared key `meta.techProvider` in `en.ts` **only** — universal program label, other locales fall back to EN (same convention as `nav.blog`). SVG path sourced from simple-icons, `fill="currentColor"` (white in hero, `gray-300` footer, `gray-500` trial bar); no image asset, nothing to translate.
- **Testimonials** (`src/data/testimonials.ts` + name-keyed overlays in `src/i18n/content/*.ts` — rename a person, update all 10 files): `Testimonials.astro` renders `avatar` as a circular `<img>` when set, else a green initials circle; shown on homepage + all trial LPs. **LogoCarousel** ("Trusted by businesses worldwide", homepage only) logos must be **flat `#374151` on transparent** (luminance→alpha from source; no photos/black boxes), and the 12-slot seamless loop needs `logos.length` to divide 12 (2/3/4/6).
...
- **`/terms/` is the single source of truth for the T&C (2026-09-23)** — the waaichat app's `/terms` route redirects here and all its in-app links open this page directly. §5 Acceptable Use carries the enumerated **Prohibited Uses** list (gambling, porn/adult, drugs, weapons, trafficking, financial crime, fraud, counterfeits, catch-all) enforced by **immediate termination without refund**; §4 covers cancellation (period-end, no refunds, revert to trial); §10 covers user deletion + the 6-month trial-inactivity purge (implemented in waaichat, cron 09:31). Any future T&C change happens HERE only — never re-create terms text in the waaichat repo (its `site_content` terms rows are a localized pointer, kept truthful via `update_terms_content.py`).
...
- **Apple Mail BOM breaks reply parsing** (cost a lost publish, PR #36, 2026-09-09): Apple Mail prefixes the quoted section with a UTF-8 BOM (`=EF=BB=BF`), so the decoded quote line is `﻿On … wrote:` — the `^On .+ wrote:` anchor in `QUOTE_RE` misses it, the whole quoted original (with its literal "YES → / NO →" instructions) scans as reply text, both keyword sets match, and the daemon ignores the reply as "ambiguous". `load_mail()` now strips `﻿`/`​` from bodies. If a YES ever "goes missing" again: check `/var/log/waai-blog-review.log` for "matched BOTH", "never notified", or "no yes/no keyword" lines BEFORE assuming the mail didn't arrive — Spark replies and Apple Mail replies format quotes differently.
...
- **Layout**: uses `BaseLayout` directly (no site header/footer — form above the fold); slim brand bar + floating WhatsApp button remain. The brand bar carries the Meta Tech Provider lockup (see Key Decisions). Not in `navigation.ts` by design.
