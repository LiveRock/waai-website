...
- **Tamil typography gotchas** (in `src/styles/global.css`, scoped via `:lang(ta)` so English is unaffected):
  - **Line-height:** Tamil vowel matras render above/below the consonant line and get clipped by tight line-heights. Tailwind's `text-*` size utilities set their own tight line-heights (`text-4xl`→1.11, `text-2xl`→1.2, `text-xs/sm`→~1.3–1.4) and `leading-tight/snug/none` clip too. `global.css` overrides these to 1.45 (large text/headings) and 1.6 (small text/tight leadings) for `:lang(ta)`. CJK gets 1.4.
  - **Word wrapping:** Tamil forms long unbroken words (no internal spaces). At large sizes a single word (e.g. வாடிக்கையாளர்களை) can exceed the line width and overflow the right edge. `:lang(ta){overflow-wrap:break-word;word-break:break-word}` lets such words wrap.
  - If Tamil ever looks clipped again, check these two first (vertical clip → line-height; horizontal/right clip → word-wrap).
